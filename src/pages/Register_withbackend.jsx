            ...
            <div className="register-card">
              {mode === "signup" ? (
                <>
                  {step === 1 && (
                    <form onSubmit={handleSubmit}>
                      ...
                      <button type="submit" className="btn btn-lg btn-primary w-100 mt-3">Next</button>
                      <div className="google-btn disabled mt-3">
                        <FcGoogle size={22} style={{ marginRight: "8px" }} />
                        Continue with Google <span className="note">(Coming soon)</span>
                      </div>
                      <p className="switch-text mt-3">
                        Already have an account?{" "}
                        <span className="switch-link" onClick={() => { setMode("signin"); setStep(1); }}>
                          Sign In
                        </span>
                      </p>
                    </form>
                      ...
                      <form onSubmit={handleSubmit}>
                        <div className="interests-grid">
                          {interestsList.map((interest, i) => (
                            <div key={i} className={`interest-card ${formData.interests.includes(interest) ? "selected" : ""}`} onClick={() => toggleInterest(interest)}>
                              {interest}
                            </div>
                          ))}
                        </div>
                        <button type="submit" className="btn btn-lg btn-primary w-100 mt-3">Finish</button>
                      </form>
                    </>
                  )}
                </>
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <input type="email" name="email" placeholder="Email Address" value={formData.email} onChange={handleChange} required />
                  <input type="password" name="password" placeholder="Password" value={formData.password} onChange={handleChange} required />
                  <button type="submit" className="btn btn-lg btn-primary w-10 mt-3">Sign In</button>
                  <p className="switch-text mt-3">
                    Don’t have an account?{" "}
                    <span className="switch-link" onClick={() => { setMode("signup"); setStep(1); }}>
                      Sign Up
                    </span>
                  </p>
                </form>
